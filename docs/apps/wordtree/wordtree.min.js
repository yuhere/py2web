...
function getURL(e, t) {
    ...
    if ("https:" === location.protocol && /^http:/.test(e))proxy(e, n); else try {
...
function change() {
    ...
    if (state = urlParams(location.search.substr(1)), state.source !== e && state.source)source.property("value", state.source), getURL(state.source, function (e) {
        processText(e)
    }), hideHelp(); else if (tokens && tokens.length) {
        var t = state.prefix;
        t || url({prefix: t = tokens[0].token}), keyword.property("value", t).node().select(), t = t.toLowerCase().match(re), treeG.call(tree.sort("occurrence" === state.sort ? function (e, t) {
...
function highlightTokens(e, t) {
    if (e)if (e.children && e.children.length)t += e.tokens.length, e.children.forEach(function (e) {
        highlightTokens(e, t)
    }); else {
...
function updateHeatmap() {
    var e = heatmap.selectAll("line").data(selectedLines), t = textViewer.size();
    ...
    d3.longscroll = function () {
        function e(e) {
            function o(o) {
                e.each(function () {
                    this.scrollTop = o;
                    var e = d3.select(this), a = 1 + Math.ceil(this.clientHeight / i), c = Math.max(0, Math.min(n - a, r)), l = c + a;
    ...
    function t() {
        function e(a) {
            ...
            if (s) {
                ...
                var p = u.enter().append("text"), g = u.selectAll("tspan").data(function (e) {
                    var t = e.tokens;
        ...
        function a(e, t, n) {
            ...
            for (var a = {}, c = {
                ...
                children: [],
                index: r[0]
            }, l = 0; i > l; l++)for (var s = r[L ? i - l - 1 : l], h = Math.min(e.length, s + C), d = Math.max(0, s - C), f = c, g = [t]; L ? --s > d : ++s < h;) {
                var x = e[s];
                if (v(x))break;
        ...
        function g(e) {
            if (L)for (var t = e.tokens[0].index, n = t - H; --t >= 0 && !v(_[t + 1]) && (t >= n || !x(_[t]));)e.tokens.unshift(_[t]); else for (var t = e.tokens[e.tokens.length - 1].index, n = t + H; ++t < _.length && !v(_[t]) && (n > t || !x(_[t - 1]));)e.tokens.push(_[t])
        }
        ...
        var y, k, w, b, T = 5, C = 10, H = 15, A = .01, L = !1, M = s, _ = [], R = [], z = !1, E = d3.dispatch("prefix"), S = d3.scale.sqrt().range([0, 25]), j = d3.layout.partition().sort(h).value(function (e) {
            ...
            return {y: e.source.x + (e.source.length ? e.source.length + T : 0), x: e.source.y}
        }).target(function (e) {
...
d3.select("#paste-go").on("click", function () {
    url(state = {source: ""}, !0), d3.select("#paste-save").property("checked") && d3.text("save").post(d3.select("#paste").property("value"), function (e, t) {
...
var page = heatmap.append("rect").datum({y: 0}).attr("class", "page").attr("width", 20).call(d3.behavior.drag().origin(Object).on("drag", drag));
d3.select(window).on("keydown.hover", hoverKey)
...
d3.select("#form").on("submit", function () {
    d3.event.preventDefault(), url({prefix: keyword.property("value")}), change()
}), 
d3.select("#form-source").on("submit", function () {
    d3.event.preventDefault(), url({source: source.property("value"), prefix: ""}, !0), change()
}), 
